...
def sanitize_for_json(text):
    """Подготовить текст для JSON без ошибок."""
    if not text: return ""
    # Убираем null-байты, управляющие символы (кроме табуляции и перевода строки)
...
def summarize_with_claude(title, article_text):
    ...
    # КРИТИЧНО: санитизируем оба текста перед JSON
    title_clean = sanitize_for_json(title)
    text_clean = sanitize_for_json(article_text[:2500])
...
Напиши коротку виважку (3-4 речення, максимум 400 символів) українською мовою. Тільки суть — що сталося, чому це важливо для бізнесу, що робити. Без вступів, без "У статті йдеться". Пиши як Bloomberg — сухо, чітко, по факту."""
    ...
    try:
        ...
        print(f"  Claude API error: HTTP {e.code}: {error_body[:200]}")
        return ""
    except Exception as e:
